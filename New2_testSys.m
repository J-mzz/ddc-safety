 function DATA = New2_testSys(Region,f,g,u,epsw,rho,psi)
...
FF_neg = matlabFunction(f-g*psi/1e-10);
FF_pos = matlabFunction(f+g*psi/1e-10);
%-------------------------Trajectory: ode45
...
%         end2
        ...
        if RHO(yprev(1),yprev(2)) <= 0 && RHO(yprev(1),yprev(2)) >= -1e-10
            f_curr = @(t, y) FF_neg(y(1),y(2)) + noise_curr;
        elseif RHO(yprev(1),yprev(2)) >= 0 && RHO(yprev(1),yprev(2)) <= 1e-10
            f_curr = @(t, y) FF_pos(y(1),y(2)) + noise_curr;
        else
            f_curr = @(t, y) FF(y(1),y(2)) + noise_curr;
        end
