...
%     sos(-Y*e +1e-6),... %2e-6; 
    ...
    sos(-rho-s2*xu -6*tol1),...
    sos(-rho-s3*xu1 -6*tol1),...
    ...
%     sos(ubound*rho -psi -1e-8*rho),...    % bound input u
%     sos(ubound*rho +psi -1e-8*rho),...
%     (sos(ubound*rho -psi)):'negative',...    % bound input u
%     (sos(ubound*rho +psi)):'positive',...
    ...
    sos(-rho*xu -psi),...    % bound input u
    sos(-rho*xu +psi),...
    ...
    sos(-rho*xu1 -psi),...    % bound input u
    sos(-rho*xu1 +psi),...
    ...
...
sol = solvesos(F, [], options, [coefficients(Y,vars);c1;c2;cr;cp;c3]);%;c3;c4;c5
out.cr = value(cr);
out.cp = value(cp);
out.sol = sol;
out.F = F;
